...
RepoLens is a standalone multi-lens code audit tool. It runs 293 expert analysis agents (205 code analysis + 18 tool gate + 14 product discovery + 26 deployment/server audit + 13 open-source readiness + 17 content quality) against any git repository or live server and creates GitHub issues for real findings. Think of it as automated code review, agent-driven pentesting, tool-driven static/dynamic analysis, and infrastructure auditing with deep specialization.
...
- **Prompts:** `prompts/_base/` (mode wrappers: audit/feature/bugfix/discover/deploy), `prompts/lenses/<domain>/<lens>.md` (293 expert prompts)
