...
RepoLens implements **Lens-Based Auditing (LBA)**, a methodology for automated code analysis that decomposes the audit problem into 293 narrow-focus specialist agents ("lenses") across 30 domains. Rather than asking a single generalist agent to review an entire codebase for every possible concern, LBA assigns each concern to a dedicated expert lens — one that examines the code through a single, specific perspective.
...
The current lens inventory spans 30 domains with 293 total lenses, broken down as: 205 code analysis + 18 tool gate + 14 product discovery + 26 deployment/server audit + 13 open-source readiness + 17 content quality.
...
LBA takes the opposite approach. By assigning one prompt per concern (293 total), each lens can devote its full context window and specialization depth to a single domain. The advantages of this decomposition include:
...
|------|---------|---------------|
| **audit** | Find real issues in existing code | 223 (code + toolgate domains) |
| **feature** | Identify missing capabilities | 223 |
| **bugfix** | Find bugs backed by evidence | 223 |
| **custom** | Change impact analysis | 223 |
