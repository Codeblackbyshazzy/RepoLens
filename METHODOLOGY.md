...
RepoLens implements **Lens-Based Auditing (LBA)**, a methodology for automated code analysis that decomposes the audit problem into 325 narrow-focus specialist agents ("lenses") across 32 domains. Rather than asking a single generalist agent to review an entire codebase for every possible concern, LBA assigns each concern to a dedicated expert lens — one that examines the code through a single, specific perspective.
...
The current lens inventory spans 32 domains with 325 total lenses, broken down as: 220 code analysis/audit-visible lenses (209 code analysis plus 11 runtime log analysis) + 18 tool gate + 14 product discovery + 43 deployment and Android audit + 13 open-source readiness + 17 content quality.
...
LBA takes the opposite approach. By assigning one prompt per concern (325 total), each lens can devote its full context window and specialization depth to a single domain. The advantages of this decomposition include:
...
|------|---------|---------------|
| **audit** | Find real issues in existing code | 238 (code + toolgate + logs domains) |
| **feature** | Identify missing capabilities | 238 |
| **bugfix** | Find bugs backed by evidence | 238 |
| **custom** | Change impact analysis | 238 |
