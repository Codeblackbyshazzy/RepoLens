...
# 6. Full parallel audit (all 284 lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A 284-lens run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 24 code/toolgate domains (214 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 24 code/toolgate domains (214 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 24 code/toolgate domains (214 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 24 code/toolgate domains (214 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (284 total across 28 domains)
...
| **Kubernetes** | 4 | Pod/container security contexts, NetworkPolicy coverage, HPA/PDB coverage, resource requests/limits, LimitRange/ResourceQuota guardrails, image tags, pull policies, registry trust |
