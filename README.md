...
Alternatives: run without installing via `npx @google/gemini-cli`, or see [geminicli.com](https://geminicli.com/) for other options. Requires Node.js 20+.

**Authenticate** — pick one:

- Run `gemini` and choose **Sign in with Google** for the browser flow (free tier: 60 requests/min, 1,000 requests/day)
- Or export `GEMINI_API_KEY` from [Google AI Studio](https://aistudio.google.com/apikey)
- Or route through Vertex AI with `GOOGLE_API_KEY` and `GOOGLE_GENAI_USE_VERTEXAI=true`

Gemini's very large context window (up to 1M tokens on `gemini-2.5-pro`) makes it well suited to auditing large codebases without splitting them across runs.
...
| `--agent-override <csv>` | Route specific domains or lenses to a different agent than the global `--agent`, trading API cost against reasoning depth — run a cheap model by default and spend a flagship model only where it matters (e.g. `security`, `architecture`). Comma-separated `key=agent` pairs. Each `key` is a domain id or a fully-qualified `domain/lens`; a bare lens id is rejected as ambiguous, since one lens id can live in more than one domain, so lens scope requires the `domain/lens` form. Each `agent` accepts the same values as `--agent` (`claude` \| `codex` \| `spark` \| `sparc` \| `opencode` \| `opencode/<model>` \| `gemini`). Precedence: `domain/lens` > `domain` > global `--agent`. The flag may be repeated; pairs accumulate. Meta agents (synthesis, triage, verification, validation, issue filing) always run on the global `--agent`. An unknown key or an invalid agent aborts startup with the offending token named. When overrides are active, the estimated cost in `--dry-run` and the confirmation prompt is broken down per agent and the active routing map is shown. Example: `--agent opencode --agent-override security=claude,architecture=claude,information-architecture/empty-states=claude`. |
...
| `REPOLENS_AGENT_TIMEOUT_GEMINI`      | unset    | Gemini per-invocation timeout override. Wins over `REPOLENS_AGENT_TIMEOUT` and mode-specific timeout variables when `--agent gemini` is selected.                                                                                                                                                                                                                                                                                                                                                             |
...
| `Invalid agent: …`                                                                                                                   | Typo in `--agent` value                                                                                      | Must be one of `claude`, `codex`, `spark`, `sparc`, `opencode`, `opencode/<model>`, `gemini`                                                                                                  |
