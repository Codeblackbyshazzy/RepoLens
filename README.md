...
| `deploy`     | 1×          | `deployment` domain (26 lenses) or `android` domain (11 lenses) | Server or Android APK audit — inspects a live server or APK target         |
...
## Domains & Lenses (308 total across 31 domains)
...
| **Android**               | `deploy`     | 11 lenses | APK overview, package metadata, bundled dependency CVE inventory, native `.so` inventory, JNI surface, binary hardening, AndroidManifest permissions and component flags, exported IPC components, intent filters, deeplinks, App Links, APK secrets, credentials, internal URLs, WebView security settings, JavaScript bridges, Network Security Config, TLS trust, certificate pinning, MITM traffic observation, logcat sensitive-data leaks, and device-aware Android audit context |
