...
# 6. Full parallel audit (all 289 lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A 289-lens run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 25 code/toolgate domains (219 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 25 code/toolgate domains (219 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 25 code/toolgate domains (219 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 25 code/toolgate domains (219 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (289 total across 29 domains)
...
| **LLM Security** | 2 | LLM output sanitization, rendering safety, prompt injection, RAG/tool/chat-history injection, markdown/link injection, external system forwarding, structured output validation, filesystem and command injection risks |
