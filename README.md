...
# 6. Full parallel audit (224 audit-visible lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A default 224-lens audit run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 26 code/toolgate domains (224 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 26 code/toolgate domains (224 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 26 code/toolgate domains (224 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 26 code/toolgate domains (224 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (294 total across 30 domains)
...
| **Infrastructure as Code** | 2 | Terraform completeness, placeholder stubs, empty resource blocks, dead modules, missing outputs, broken references, provider/backend hygiene, Terraform security groups, encryption, IAM, public access, disabled resources, zero-resource plans, README-vs-code infrastructure promises |
