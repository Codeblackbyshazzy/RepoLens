...
assert_contains "logs dry-run shows five lenses" "Lenses:       5" "$logs_dry_output"
...
assert_eq "logs domain has five lenses" "5" "$logs_lens_count"
logs_lens_ids="$(jq -r '.domains[] | select(.id == "logs") | .lenses | join(",")' "$SCRIPT_DIR/config/domains.json")"
assert_eq "logs domain registers expected lenses" "error-storms,error-cascades,retry-loops,recursive-growth,log-gaps" "$logs_lens_ids"
