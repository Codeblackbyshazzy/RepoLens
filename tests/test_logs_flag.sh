...
assert_contains "logs dry-run shows eighteen lenses" "Lenses:       18" "$logs_dry_output"
...
assert_eq "logs domain has eighteen lenses" "18" "$logs_lens_count"
logs_lens_ids="$(jq -r '.domains[] | select(.id == "logs") | .lenses | join(",")' "$SCRIPT_DIR/config/domains.json")"
assert_eq "logs domain registers expected lenses" "error-storms,error-cascades,retry-loops,recursive-growth,resource-leaks,resource-exhaustion,log-gaps,missing-heartbeats,silent-failures,state-machine-violations,state-corruption,race-conditions,lifecycle-violations,orphaned-events,process-orphans,latency-degradation,clock-skew,timeout-clusters" "$logs_lens_ids"
